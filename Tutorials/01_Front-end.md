...
2. Go to the dashboard and look for the option that say "New sandbox". Select it.
3. From the Quick start tab, select the one that says "React", note that there is an option that says "React TypeScript", we are not interested in this one so do not confuse it.
...
```
- Remember to check the documentation every time you can to know about the methods and life cycle of the component :)
...
```
- if you save, we still will  have an error in the browser since we haven't export our class object yet.
8. Export your class at the end of the code
``` javascript
export default App
```
- After saving, you will see how the browser displays our string.
-  Let's make something more fancy by introducing a new concept: JSX <https://reactjs.org/docs/introducing-jsx.html>.
...
```
- Save and visualize the outcome (sometimes you need to reload the webpage to see the result)
- The structure of our simple CMS is there, it has a *div* dedicated for the menu and a *div* for the viewer. As you can observer, I have already declared some classes for the style, currently they are inactive since we have not attached the framework to our app. Such classes come from a framework named, [Bootstraps](https://getbootstrap.com/docs/5.1/getting-started/introduction/). You can learn more about them in the official web page.
...
```
- if you reload your project you should be able to get a nav bar tha look more or less like this
...
```
- Save and reload and you will see how all the view of every section will look.
- Our code is getting a little bit complicated and handle it properly will get hard. So let's make each view of the viewer a *Component* so we can make our code a little bit more modular.


4. In *src* folder create a new folder and name it ***CustomComponent***


5. Inside ***CustomComponent*** create 7 empty javascript documents. You can name them as you wish but to follow this tutorial I recommend you to assig these names:
...
```
8. After fully completed step 6 and 7, delete all content between the *div* with the *id* *viewer*. Instead place ***one*** of the custom components we have created.
```javascript
<div id="viewer" className="row container">
 <HomeView />
</div>
```
9. Save and reload. Are you able to see the content of the component? If yes, you can continue to the next part of the tutorial;  otherwise check what are you missing from the previous steps.
...
3. Create a method that receives the *state* as parameter and returns a *view*
```javascript
...
```
- So far not too much has change, we still should be able to see the content of *HomeView* component as it is the default value
...
- In this case we are updating the *CurrenPage* and we are assigning the value of the property *page*
...
```
- As you may remember, you can assign prop to any react component.
- Our prop in this case will receive a value from the child and we will invoke the *QSetView* method to define the view
...
- The remaining thing is to connect it with our back-end and prepare it to deploy it.


For now zip everything we have done today and submit it to e-classroom/push it to the repo.
Next session, in order to finish our project it is mandatory to finalise the back-end accordingly to the previous tutorials.
