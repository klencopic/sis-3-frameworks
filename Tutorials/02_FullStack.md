...
So far  in these set of tutorials we have: i) built a server using **Express.js**, ii) established a connections with the db using **mysql** and iii) built the client(front-end) using **React.js**.
...
- <strike>Create a proxy so we don't have to change the endpoints while building the final version of our client</strike>
- Create the logic in the client side to handle the responses from the server


### CORS
1. Install CORS dependency. If we don't do this step we wont be able to retrieve data from the server in the client server
...
- data prop is used to assign the current id that comes from a callback from **NoviceView**
...
```
3. Add to every input tag the attribute name and assign the corresponding value, for example:
```javascript
...
5. Add the prop on change to each input element in out JSX
```javascript
...
#### Login
For login, we have to repeat same step 1-7 from previous views(AddNovicaView) but using the corresponding name variables.
...
```
3. Add the prop on change to each input element in out JSX
...
5. In ***App.js*** add the corresponding prop to *LogingView* component to receive the call back and set in the local state the user status, just if we have received a positive response.


```javascript
<LoginView QUserFromChild={this.QSetUser}/>
```
6. Create the method to set the local state, so we don't need to refresh the page:
```javascript
QSetUser=(obj)=>{
 this.setState({
...
- Make a conditional render for a logout button and logout the session.
...
2. Stop your *express* server and copy the folder build into the root of the back.end project.
3. If you have not, install the path dependency in the back-end project and import it in *index.js*
```console
npm install path
```
4. In *index.js* in  the back-end add the following lines_
...
- We ar using express.static to point to the folder we just copied from react, this will warranty we will return the index.html inside that folder.
- Check in you browser, you react-app now is hosted inside the express server.
- Now your web pager/service is online. Unfortunately, it will stop once you logout from ssh. To avoid this, let use a daemon service to run forever our application, or at least till the sys admin allows it. :)
...
```
- Just make sure you are in the root of your project and that your entry file is named index.js
